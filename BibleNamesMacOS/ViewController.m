//
//  ViewController.m
//  BibleNamesMacOS
//
//  Created by Robert Rahardja on 24/2/17.
//  Copyright © 2017 Robert Rahardja. All rights reserved.
//

#import "ViewController.h"

@interface ViewController ()
//@property (weak, nonatomic) IBOutlet UITextField *InputText;
@property (weak) IBOutlet NSTextField *InputText;

//@property (weak, nonatomic) IBOutlet UITextView *OutputText;
@property (unsafe_unretained) IBOutlet NSTextView *OutputText;




@property (nonatomic,strong) NSDictionary *dict;
@property (nonatomic,strong) NSMutableString *string;
@end


@implementation ViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    //self.dict = @{ @"Jarvah": @"breathing", @"Mishal": @" parables governing"};
    self.dict = @{ @"Jarvah": @"breathing",
                   @"Mishal": @" parables governing",
                   @"Pagiel": @"",
                   @"Matri": @"",
                   @"Abel": @"-shittim meadow of the acacias",
                   @"Beeroth": @"",
                   @"Ophel": @"",
                   @"Shinar": @"",
                   @"Jozabad": @"",
                   @"Shelemiah": @"",
                   @"Benhanan": @"",
                   @"Meroz": @"",
                   @"Pelethites": @"",
                   @"Athlai": @"",
                   @"Hatach": @"",
                   @"Chozeba": @"",
                   @"Gozan": @"",
                   @"Iphedeiah": @"may God redeem YHWH does deliver redemption",
                   @"Eshek": @"",
                   @"Mithcah": @"",
                   @"Sceva": @"",
                   @"Elmodam": @"",
                   @"Joseph": @"remover or increaser increase may God add",
                   @"Malchiel": @"",
                   @"Reuel": @"",
                   @"Judas": @"",
                   @"Rome": @"",
                   @"Shiphrah": @"",
                   @"Shuphim": @"",
                   @"Etham": @"",
                   @"Sepharvaim": @"",
                   @"Lachish": @"",
                   @"Elias": @"",
                   @"Peresh": @"",
                   @"Manahethites": @"",
                   @"Simeon": @"God hears me",
                   @"Yoav": @"",
                   @"Onesimus": @"",
                   @"Baca": @"",
                   @"Cyrenius": @"",
                   @"Mered": @"",
                   @"Iconium": @"",
                   @"Esmachiah": @"",
                   @"Ziba": @"",
                   @"Ziza": @"",
                   @"Philippi": @"",
                   @"Hattill": @"",
                   @"Regemmelech": @"",
                   @"Abiathar": @"father of abundance i.e. liberal father of abundance or my father excels",
                   @"Eubulus": @"",
                   @"Uthai": @"",
                   @"Nebai": @"",
                   @"Beera": @"",
                   @"Persia": @"",
                   @"Shobai": @"",
                   @"Apharsathchites": @"etymology uncertain",
                   @"Rezon": @"",
                   @"Abib": @"an ear of corn green fruits",
                   @"Harum": @"",
                   @"Zelah": @"",
                   @"Bethshan": @"",
                   @"Jekamiah": @"whom YHWH gathers",
                   @"Careah": @"bald",
                   @"Melchizedek": @"",
                   @"Buz": @"",
                   @"Giah": @"",
                   @"Hur": @"",
                   @"Phallu": @"",
                   @"Mahali": @"",
                   @"Noph": @"",
                   @"Joshah": @"",
                   @"Jeroham": @"",
                   @"Kerioth": @"",
                   @"Ahishar": @"",
                   @"Meremoth": @"",
                   @"Nephish": @"",
                   @"Pasdammin": @"",
                   @"Argob": @"",
                   @"Bilhah": @"",
                   @"Sadoc": @"",
                   @"Prochorus": @"",
                   @"Boskath": @"",
                   @"Serug": @"",
                   @"Asareel": @"",
                   @"Bithynia": @"",
                   @"Samaria": @"",
                   @"Hirah": @"",
                   @"Ephratah": @"",
                   @"Seth": @"",
                   @"Ucal": @"",
                   @"Dagon": @"",
                   @"Ahab": @"uncle",
                   @"Justus": @"just",
                   @"Asuppim": @"house of gatherings",
                   @"Kallai": @"",
                   @"Hagabah": @"grasshopper",
                   @"Elizur": @"",
                   @"Sinim": @"",
                   @"Sarid": @"",
                   @"Nahor": @"",
                   @"Migdol": @"",
                   @"Parmenas": @"",
                   @"Mushi": @"Drawn Out  Proved Of The Lord",
                   @"Pelonite": @"",
                   @"Misrephoth": @"-maim",
                   @"Hamor": @"",
                   @"Adoni": @"-zedek",
                   @"Caphtor": @"",
                   @"Atad": @"a thorn",
                   @"Eliam": @"",
                   @"Jahaleel": @"",
                   @"Magog": @"region of Gog",
                   @"Rakkon": @"",
                   @"Mnason": @"",
                   @"Halah": @"",
                   @"Jokdeam": @"",
                   @"Jahdiel": @"union of God or God makes glad",
                   @"Ebiasaph": @"",
                   @"Enos": @"",
                   @"Egypt": @"",
                   @"Ummah": @"",
                   @"Cherith": @"",
                   @"Asenath": @"worshipper of Neith",
                   @"Chenaanah": @"",
                   @"Nezib": @"",
                   @"Attalia": @"from Attalus",
                   @"Nebuchadrezzar": @"",
                   @"Narcissus": @"",
                   @"Darkon": @"bearer or scattering possibly related to Aramaic terms for hasten or shield",
                   @"Dimon": @"",
                   @"Rachel": @"",
                   @"Smyrna": @"",
                   @"Daberath": @"",
                   @"El": @"-elohe-Israel",
                   @"Jeconiah": @"",
                   @"Telah": @"",
                   @"Jair": @"YHWH enlightens arouses or who diffuses light",
                   @"Shedeur": @"",
                   @"Semaiah": @"",
                   @"Tyrus": @"",
                   @"Bukki": @"",
                   @"Bidkar": @"",
                   @"Ivah": @"",
                   @"Bajith": @"",
                   @"Zarthan": @"",
                   @"Paran": @"",
                   @"Israel": @"he strives with God ruling with God",
                   @"Pisidia": @"",
                   @"Jazer": @"YHWH helps",
                   @"Elijah": @"",
                   @"Eleph": @"",
                   @"Ahikam": @"a brother who raises up",
                   @"Quartus": @"",
                   @"Lystra": @"",
                   @"Zuph": @"",
                   @"Ibri": @"Hebrew passing over of a Hebrew",
                   @"Jael": @"",
                   @"Semachiah": @"",
                   @"Aaron": @"a teacher or lofty bright shining (etymology doubtful)",
                   @"Gaddi": @"",
                   @"Jezrahiah": @"produced by YHWH a leader of the choir",
                   @"Mozah": @"",
                   @"Gershom": @"",
                   @"Ziphims": @"",
                   @"Sion": @"",
                   @"Jerubbaal": @"",
                   @"Zarah": @"",
                   @"Mithnite": @"",
                   @"Miniamin": @"",
                   @"Jahaziel": @"whom God watches over beheld by God God sees or reveals",
                   @"Cain": @"acquisition fabrication",
                   @"Manasseh": @"forgetting who makes to forget God hath made me forget",
                   @"Nazareth": @"",
                   @"Igdaliah": @"may God be glorified great is YHWH",
                   @"Nahshon": @"",
                   @"Hermogenes": @"",
                   @"Buzi": @"",
                   @"Sem": @"",
                   @"Jeshebeab": @"May The Father Tarry (Live), Habitation Of The Father",
                   @"Evil": @"-merodach",
                   @"Benoni": @"",
                   @"Dalmatia": @"",
                   @"Casluhim": @"",
                   @"Kadmiel": @"",
                   @"Jeruel": @"",
                   @"Uz": @"",
                   @"Semei": @"",
                   @"Helek": @"",
                   @"Nisroch": @"",
                   @"Amasa": @"",
                   @"Adamah": @"red earth",
                   @"Pasach": @"",
                   @"Asia": @"",
                   @"Minneiah": @"",
                   @"Lekah": @"",
                   @"Chiliab": @"",
                   @"Jotham": @"",
                   @"Nicodemus": @"",
                   @"Gilead": @"",
                   @"Pirathon": @"",
                   @"Hamutal": @"",
                   @"Titus": @"",
                   @"Hadashah": @"",
                   @"Sabtechah": @"",
                   @"Zaavan": @"terror",
                   @"Bahurim": @"",
                   @"Jered": @"",
                   @"Naarah": @"",
                   @"Magdala": @"",
                   @"Melchi": @"-shua",
                   @"Zephaniah": @"",
                   @"Zabbud": @"given gift",
                   @"Satan": @"",
                   @"Cyrus": @"",
                   @"Taphath": @"",
                   @"Obed": @"-edom",
                   @"Madmannah": @"",
                   @"Cis": @"",
                   @"Ammah": @"",
                   @"Ilai": @"exaltedsupreme",
                   @"Lois": @"",
                   @"Kishion": @"",
                   @"Joah": @"",
                   @"Jehiskiah": @"",
                   @"Pau": @"",
                   @"Nob": @"",
                   @"Benhadad": @"",
                   @"Jaasiel": @"whom God comforts made by God God is maker",
                   @"Segub": @"",
                   @"Hushah": @"",
                   @"Sheva": @"",
                   @"Malcham": @"",
                   @"Chislon": @"",
                   @"Sechu": @"",
                   @"Sorek": @"",
                   @"Iscariot": @"a man of murder a hireling man of kerioth",
                   @"Zibeon": @"",
                   @"Padon": @"",
                   @"Haman": @"",
                   @"Ramathaim": @"-Zophim",
                   @"Elasah": @"",
                   @"Maath": @"",
                   @"Diotrephes": @"",
                   @"Beriah": @"",
                   @"Timotheus": @"",
                   @"Antipas": @"",
                   @"Omega": @"",
                   @"Iram": @"watchful",
                   @"Ijon": @"look",
                   @"Jeshimon": @"",
                   @"Jeduthun": @"",
                   @"Sardites": @"",
                   @"Doeg": @"",
                   @"Zorathites": @"",
                   @"Apelles": @"",
                   @"Laban": @"",
                   @"Job": @"persecuted",
                   @"Nehelamite": @"",
                   @"Zatthu": @"",
                   @"Sinon": @"",
                   @"Susi": @"",
                   @"Philadelphia": @"",
                   @"Tekoa": @"",
                   @"Uel": @"",
                   @"Nibhaz": @"the barker",
                   @"Salcah": @"",
                   @"Nergal": @"-sharezer",
                   @"Ammishaddai": @"",
                   @"Maachah": @"",
                   @"Paltiel": @"",
                   @"Eliphal": @"",
                   @"Libni": @"White",
                   @"Pure":@"",
                   @"Nahallal": @"",
                   @"Shiphi": @"",
                   @"Chuza": @"",
                   @"Mesha": @"",
                   @"Jaazaniah": @"whom YHWH hears YHWH does hear  may God hear",
                   @"Kushaiah": @"",
                   @"Ahi": @"my brother my brethren",
                   @"Sippai": @"",
                   @"Azaniah": @"Yahweh listened",
                   @"Hanun": @"",
                   @"Hivites": @"",
                   @"Mehir": @"",
                   @"Hakupha": @"",
                   @"Christian": @"'brethren' 'the faithful' 'elect' 'saints' 'believers' to the followers of Iesus",
                   @"Naharai": @"",
                   @"Spain": @"",
                   @"Shitrai": @"",
                   @"Jerioth": @"",
                   @"Mehujael": @"",
                   @"Gob": @"",
                   @"Neiel": @"",
                   @"Japhia": @"",
                   @"Rufus": @"",
                   @"Shaalabbim": @"",
                   @"Menan": @"",
                   @"Hosah": @"",
                   @"Dimonah": @"",
                   @"Heshbon": @"",
                   @"Jesiah": @"",
                   @"Zelotes": @"",
                   @"Joktan": @"",
                   @"Asher": @"Happy am I",
                   @"Armageddon": @"the hill or city of Megiddo",
                   @"Tirzah": @"",
                   @"Persis": @"",
                   @"Mekonah": @"",
                   @"Paarai": @"",
                   @"Hosea": @"",
                   @"Mitylene": @"",
                   @"Amnon": @"faithful",
                   @"Meunim": @"",
                   @"Rephidim": @"",
                   @"Merom": @"",
                   @"Alvah": @"",
                   @"Olympas": @"",
                   @"John": @"YHWHs gift: @the same name as Johanan a contraction of Jehohanan",
                   @"Gittah": @"-hepher",
                   @"Bechorath": @"",
                   @"Tyrannus": @"",
                   @"Shammai": @"",
                   @"Zin": @"",
                   @"Raguel": @"",
                   @"Zohar": @"Gray Whiteness",
                   @"Demas": @"ruler of people",
                   @"Jashobeam": @"",
                   @"Augustus": @"venerable",
                   @"Samothracia": @"",
                   @"Golan": @"",
                   @"Behemoth": @"",
                   @"Daniel": @"",
                   @"Zer": @"",
                   @"Ruth": @"",
                   @"Seorim": @"Barely",
                   @"Jezoar": @"",
                   @"Dabareh": @"",
                   @"Massa": @"burden a lifting up gift",
                   @"Kehelahath": @"",
                   @"Bethel": @"",
                   @"Shealtiel": @"",
                   @"Besai": @"",
                   @"Samuel": @"",
                   @"Diklah": @"",
                   @"Gispa": @"",
                   @"Rissah": @"",
                   @"Baale": @"",
                   @"Pison": @"",
                   @"Phalec": @"",
                   @"Ruhamah": @"",
                   @"Pudens": @"",
                   @"Shemiramoth": @"",
                   @"Avith": @"",
                   @"Thaddeus": @"",
                   @"Annas": @"humble",
                   @"Amos": @"burden",
                   @"Hallelujah": @"",
                   @"Hormah": @"",
                   @"Jared": @"",
                   @"Gaash": @"",
                   @"Eph": @"-lal",
                   @"Palti": @"",
                   @"Shittim": @"",
                   @"Hethlon": @"",
                   @"Jahdo": @"union",
                   @"Hebron": @"Confederation Association league",
                   @"Eri": @"",
                   @"Chun": @"",
                   @"Bethemek": @"",
                   @"Aijeleth": @"-Shahar",
                   @"Ahimoth": @"brother of death",
                   @"Mary": @"",
                   @"Chelal": @"",
                   @"Adrammelech": @"splendor of the king",
                   @"Cenchrea": @"",
                   @"Jahaz": @"",
                   @"Zereda": @"",
                   @"Kamon": @"",
                   @"Gur": @"-baal",
                   @"Zeboim": @"",
                   @"Machir": @"",
                   @"Cephas": @"",
                   @"Bezer": @"",
                   @"Eliah": @"",
                   @"Saphir": @"",
                   @"Mashal": @"",
                   @"Bedaiah": @"",
                   @"Zorobabel": @"",
                   @"Geshur": @"Geshuri",
                   @"Sardis": @"",
                   @"Eliezer": @"",
                   @"Michmethah": @"",
                   @"Lubin": @"",
                   @"Aharah": @"",
                   @"Zeror": @"",
                   @"Elidad": @"",
                   @"Achish": @"",
                   @"Noha": @"",
                   @"Nodab": @"",
                   @"Ramiah": @"",
                   @"Jesse": @"",
                   @"Jokshan": @"",
                   @"Peninnah": @"",
                   @"Abiram": @"father of height i.e. proud",
                   @"Theudas": @"",
                   @"Jehucal": @"",
                   @"Linus": @"",
                   @"Kedar": @"",
                   @"Ebal": @"",
                   @"Eliud": @"",
                   @"Mibzar": @"",
                   @"Eglaim": @"",
                   @"Shuham": @"",
                   @"Binea": @"",
                   @"Pekod": @"",
                   @"Beri": @"",
                   @"Sennacherib": @"",
                   @"Elpalet": @"",
                   @"Hoglah": @"",
                   @"Shinab": @"",
                   @"Carcas": @"",
                   @"Chilmad": @"",
                   @"Artemas": @"",
                   @"Tarpelites": @"",
                   @"Haradah": @"",
                   @"Bashemath": @"",
                   @"Ulai": @"",
                   @"Mephibosheth": @"",
                   @"Hosanna": @"",
                   @"Jew": @"a man of Judea",
                   @"Achsah": @"",
                   @"Ishbah": @"praising He praises appeaser",
                   @"Hothir": @"",
                   @"Stephen": @"",
                   @"Amzi": @"strong",
                   @"Haniel": @"",
                   @"Mispar": @"",
                   @"Zedad": @"",
                   @"Zurishaddai": @"",
                   @"Appaim": @"nostrils",
                   @"Matthal": @"",
                   @"Mearah": @"",
                   @"Shenazar": @"",
                   @"Malchus": @"",
                   @"Pai": @"Pau blessing",
                   @"Rezin": @"",
                   @"Jecamiah": @"whom YHWH gathers",
                   @"Archelaus": @"the prince of the people",
                   @"Deuteronomy": @"",
                   @"Nahath": @"",
                   @"Baasha": @"boldness offensive he who lays waste",
                   @"Zemarite": @"",
                   @"Zior": @"",
                   @"Shen": @"",
                   @"Bathsuha": @"",
                   @"Berachiah": @"",
                   @"Hori": @"",
                   @"Sisamai": @"",
                   @"Shamayim": @"",
                   @"Nineveh": @"",
                   @"Hara": @"",
                   @"Berea": @"",
                   @"Aminadab": @"",
                   @"Judith": @"",
                   @"Jada": @"wise",
                   @"Kanah": @"",
                   @"Areopagus": @"",
                   @"Shephatiah": @"",
                   @"Zaanaim": @"removings",
                   @"Asherah": @"",
                   @"Pelatiah": @"",
                   @"Cabul": @"",
                   @"Ithiel": @"God is with me God is God is with me",
                   @"Idumea": @"",
                   @"Hezer": @"",
                   @"Heres": @"",
                   @"Palestina": @"",
                   @"Pentapolis": @"",
                   @"Shual": @"",
                   @"Abdon": @"servile",
                   @"Mithredath": @"",
                   @"Haggeri": @"",
                   @"Sabtah": @"",
                   @"Jokneam": @"",
                   @"Canaan": @"Merchant or Servant.",
                   @"Athach": @"",
                   @"Abihud": @"father of renown famous father (i.e. possessor) of renown",
                   @"Eliada": @"",
                   @"Isshijah": @"there is God",
                   @"Leah": @"",
                   @"Ariel": @"lion of God",
                   @"Socoh": @"",
                   @"Apocalypse": @"revelation",
                   @"Pentateuch": @"",
                   @"Zadok": @"just righteous",
                   @"Jekamean": @"",
                   @"Jehu": @"",
                   @"Joram": @"",
                   @"Zenan": @"",
                   @"Ishvah": @"resembles",
                   @"Achor": @"",
                   @"Azubah": @"forsaken",
                   @"Helkai": @"",
                   @"Gamaliel": @"",
                   @"Zeruiah": @"",
                   @"Zilthai": @"",
                   @"Salamis": @"",
                   @"Esaias": @"",
                   @"Hamon": @"-gog",
                   @"Shehariah": @"",
                   @"Hakkatan": @"",
                   @"Sivan": @"",
                   @"Ithai": @"God is with me",
                   @"Abaddon": @"see Apollyon a destroyer",
                   @"Bocheru": @"",
                   @"Alush": @"",
                   @"Beersheba": @"",
                   @"Thomas": @"",
                   @"Bashan": @"",
                   @"Dodavah": @"",
                   @"Shimeon": @"",
                   @"Gazzam": @"",
                   @"Josaphat": @"",
                   @"Apollyon": @"a destroyer angel of the bottomless pit",
                   @"Shilhi": @"",
                   @"Secundus": @"",
                   @"Jedaiah": @"Known Of The Lord",
                   @"Harim": @"Consecrated To God",
                   @"Shachia": @"",
                   @"Ezra": @"",
                   @"Arah": @"wayfaring",
                   @"Helon": @"",
                   @"Jakim": @"God/He Will Set/lift/raise Him up.",
                   @"Dara": @"misspelling of pearl of wisdom Darda",
                   @"Obal": @"",
                   @"Aharhel": @"",
                   @"Diana": @"",
                   @"Rehpaim": @"",
                   @"Kithlish": @"",
                   @"Pamphylia": @"",
                   @"Thelasar": @"",
                   @"Sisera": @"",
                   @"Gareb": @"",
                   @"Sarsechim": @"",
                   @"Sotai": @"",
                   @"Phanuel": @"",
                   @"Zorah": @"",
                   @"Zebudah": @"",
                   @"Dionysius": @"",
                   @"Coniah": @"",
                   @"Baalim": @"",
                   @"Zeruah": @"",
                   @"Pontus": @"",
                   @"Shihon": @"sound",
                   @"Irijah": @"may God see God does see provide fear of the Lord",
                   @"Dishon": @"",
                   @"Hanoch": @"Initiated Dedicated",
                   @"Pethuel": @"",
                   @"Saron": @"",
                   @"Shaphat": @"",
                   @"Middin": @"",
                   @"Michael": @"",
                   @"Mash": @"",
                   @"Bered": @"",
                   @"Sanballat": @"",
                   @"Sepharad": @"",
                   @"Edrei": @"",
                   @"Sihon": @"",
                   @"Naum": @"",
                   @"Akeldama": @"",
                   @"Baalath": @"-Beer Baalah of the well",
                   @"Nehushtan": @"",
                   @"Jeezer": @"",
                   @"Nebat": @"",
                   @"Joatham": @"",
                   @"Perga": @"",
                   @"Hod": @"",
                   @"Miriam": @"rebellion rebellous bitterness their rebellion",
                   @"Shuthelah": @"",
                   @"Berothath": @"",
                   @"Hammedatha": @"",
                   @"Tiphsah": @"",
                   @"Jibsam": @"",
                   @"Mesobaite": @"",
                   @"Thummim": @"",
                   @"Ara": @"",
                   @"Arad": @"a wild ass",
                   @"Hadarezer": @"",
                   @"Ziddim": @"",
                   @"Zelzah": @"",
                   @"Abimelech": @"father of the king my father a king or father of a king",
                   @"Gatam": @"",
                   @"Makheloth": @"",
                   @"Achshaph": @"",
                   @"Iim": @"",
                   @"Irpeel": @"",
                   @"Mizar": @"",
                   @"Antothijah": @"answers of YHWH",
                   @"Hul": @"",
                   @"Zelophehad": @"",
                   @"Telabib": @"",
                   @"Zimmah": @"",
                   @"Pedaiah": @"",
                   @"Arioch": @"",
                   @"Shobab": @"",
                   @"Jiphthael": @"",
                   @"Jehubbah": @"",
                   @"Aiah": @"",
                   @"Phichol": @"",
                   @"Accho": @"",
                   @"Exodus": @"going out departure",
                   @"Ahlab": @"",
                   @"Paphos": @"",
                   @"Oreb": @"",
                   @"Elishua": @"",
                   @"Millo": @"",
                   @"Zoba": @"",
                   @"Remphan": @"",
                   @"Ahishahar": @"the  brother is dawning light",
                   @"Birsha": @"",
                   @"Metheg": @"-ammah",
                   @"Zedekiah": @"",
                   @"Jemuel": @"Gods day son of God",
                   @"Shaveh": @"",
                   @"Misti": @"",
                   @"Shechem": @"",
                   @"Ahlai": @"beseeching sorrowing expecting",
                   @"Thebez": @"",
                   @"Barsabas": @"son of Sabas or rest",
                   @"Josibiah": @"",
                   @"Immer": @"saying speaking a lamb talkative prominent",
                   @"Berith": @"",
                   @"Arab": @"",
                   @"Tatnai": @"",
                   @"Salmon": @"",
                   @"Jehoadah": @"whom YHWH adorns",
                   @"Lysias": @"",
                   @"Salma": @"",
                   @"Perazim": @"",
                   @"Eagle": @"a tearer with the beak properly the griffon vulture or great vulture so called from its tearing its prey with its beak",
                   @"Zebul": @"",
                   @"Nedabiah": @"",
                   @"Malchijah": @"Yahweh Is King,King Of The Lord(?)",
                   @"Zabdiel": @"gift of God",
                   @"Erastus": @"",
                   @"Zobebah": @"",
                   @"Agee": @"",
                   @"Amorite": @"",
                   @"Jogli": @"",
                   @"Anah": @"one who answers",
                   @"Imnah": @"may God defend prosperity he allots",
                   @"Abitub": @"father of goodness",
                   @"Elymas": @"",
                   @"Stachys": @"",
                   @"Nod": @"",
                   @"Silvanus": @"",
                   @"Moses": @"to draw drawn drawn forth taken out of water or a son",
                   @"Jadon": @"he that rules or abids",
                   @"Sigionoth": @"",
                   @"Aphiah": @"",
                   @"Karkor": @"",
                   @"Gammadims": @"",
                   @"Arabia": @"barren desert",
                   @"Benaiah": @"",
                   @"Ard": @"one that descending descent",
                   @"Zaccur": @"mindful",
                   @"Archippus": @"master of the horse",
                   @"Ephesus": @"",
                   @"Areli": @"",
                   @"Geuel": @"",
                   @"Silla": @"",
                   @"Girgashite": @"",
                   @"Saph": @"",
                   @"Ahimaaz": @"",
                   @"Rabbith": @"multitude",
                   @"Hattush": @"",
                   @"Syracuse": @"",
                   @"Romamti": @"-ezer",
                   @"Ezion": @"-Geber",
                   @"Nabal": @"",
                   @"Phurah": @"",
                   @"Jecoliah": @"able through YHWH The same as Jecholiah: @strong through YHWH",
                   @"Lebonah": @"",
                   @"Uphaz": @"",
                   @"Gennesaret": @"",
                   @"Perizzites": @"",
                   @"Zareathites": @"",
                   @"Shiggaion": @"",
                   @"Pontius": @"",
                   @"Jaaziah": @"God consoles or determines  may God strengthen",
                   @"Jehoiarib": @"Yahweh pleads or contends",
                   @"Rab": @"-saris",
                   @"Blastus": @"",
                   @"Mahaz": @"",
                   @"Tochen": @"",
                   @"Ashima": @"",
                   @"Gog": @"",
                   @"Pithom": @"",
                   @"Shillem": @"",
                   @"Ishbak": @"free empty exhausted",
                   @"Meronothite": @"",
                   @"Imna": @"God does restrain withdrawing",
                   @"Jiphtah": @"",
                   @"Barjesus": @"son of Iesus wise",
                   @"Shemuel": @"",
                   @"Anak": @"",
                   @"Ner": @"",
                   @"Phinehas": @"",
                   @"Ishvi": @"quiet*Ishvah resembles",
                   @"Rabbah": @"",
                   @"Jamlech": @"",
                   @"Kish": @"",
                   @"Shibboleth": @"",
                   @"Joppa": @"beauty",
                   @"Besodeiah": @"",
                   @"Sheshan": @"",
                   @"Phrygia": @"",
                   @"Bizjothjas": @"",
                   @"Eliab": @"",
                   @"Samson": @"",
                   @"Beraiah": @"",
                   @"Sosthenes": @"",
                   @"Mizraim": @"",
                   @"Zuzims": @"",
                   @"Tishbite": @"",
                   @"Eker": @"",
                   @"Vaniah": @"",
                   @"Abinadab": @"father of nobleness i.e. noble",
                   @"Absalom": @"father of peace",
                   @"David": @"beloved",
                   @"Zidonians": @"",
                   @"Shemariah": @"",
                   @"Sebat": @"",
                   @"Pisgah": @"",
                   @"Serah": @"",
                   @"Jeuel": @"",
                   @"Taanach": @"-shilo",
                   @"Jeshishai": @"",
                   @"Terah": @"",
                   @"Adbeel": @"",
                   @"Talmai": @"",
                   @"Nebushasi": @"hahban",
                   @"Jokim": @"",
                   @"Schaaph": @"",
                   @"Joshbekesha": @"",
                   @"Zaretan": @"",
                   @"Bozez": @"",
                   @"Ephes": @"-dammim",
                   @"Jakan": @"",
                   @"Shuah": @"",
                   @"Damascus": @"",
                   @"Machpelah": @"",
                   @"Hashum": @"",
                   @"Aedias": @"",
                   @"Uzzah": @"",
                   @"Jehoram": @"",
                   @"Jobab": @"",
                   @"Lahad": @"",
                   @"Ahiezer": @"",
                   @"Lashah": @"",
                   @"Joelah": @"",
                   @"Eglah": @"",
                   @"Mark": @"the evangelist",
                   @"Huzzab": @"",
                   @"Ephraim": @"Two-fold Increase",
                   @"Beer": @"-lahai-roi",
                   @"Leor": @"",
                   @"Laadah": @"",
                   @"Adami": @"my man earth",
                   @"Eder": @"",
                   @"Adam": @"red earth",
                   @"Shushan": @"",
                   @"Ecclesiastes": @"",
                   @"Abida": @"(or Abidah)",
                   @"Ar": @"",
                   @"Nisan": @"",
                   @"Neballat": @"",
                   @"Ithmah": @"purity bereavement",
                   @"Sarah": @"",
                   @"Jesus": @"The Greek form of the name Joshua or Jeshua a contraction of Jehoshua that is help of YHWH or saviour.",
                   @"Jabbok": @"",
                   @"Mehida": @"",
                   @"Benzoheth": @"",
                   @"Jaazah": @"YHWH helps",
                   @"Malachi": @"",
                   @"Jethro": @"",
                   @"Eber": @"",
                   @"Rehum": @"",
                   @"Gihon": @"",
                   @"Jegar": @"-sahadutha",
                   @"Shiloah": @"",
                   @"Lycaonia": @"",
                   @"Hananeel": @"",
                   @"Bakbakkar": @"diligent searcher",
                   @"Jehud": @"",
                   @"Eshbaal": @"",
                   @"Zethan": @"",
                   @"Ophrah": @"",
                   @"Hanani": @"",
                   @"Gibbethon": @"",
                   @"Mahanaim": @"",
                   @"Gudgodah": @"",
                   @"Uri": @"",
                   @"Kidron": @"",
                   @"Tilon": @"",
                   @"Shammoth": @"",
                   @"Helkath": @"-hazzurim",
                   @"Naam": @"",
                   @"Zebulunites": @"",
                   @"Taralah": @"",
                   @"Demetrius": @"",
                   @"Rimmon": @"",
                   @"Azur": @"",
                   @"Pentecost": @"",
                   @"Tiberius": @"",
                   @"Megiddo": @"",
                   @"Zaanannim": @"",
                   @"Bozrah": @"",
                   @"Ohel": @"",
                   @"Elihu": @"",
                   @"Tabbaoth": @"rings",
                   @"Marsena": @"",
                   @"Pekahiah": @"",
                   @"Ammon": @"",
                   @"Midian": @"",
                   @"Nogah": @"",
                   @"Eshcol": @"",
                   @"Kinah": @"",
                   @"Aquila": @"an eagle",
                   @"Hagab": @"",
                   @"Ahaz": @"one that takes or possesses",
                   @"Zetham": @"",
                   @"Jaaziel": @"God is determining or consoling may God strengthen",
                   @"Junia": @"",
                   @"Ahinadab": @"",
                   @"Ahumai": @"brother of water i.e. cowardly",
                   @"Adria": @"",
                   @"Hoham": @"",
                   @"Machi": @"decrease",
                   @"Rhodoks": @"",
                   @"Tehinnah": @"",
                   @"Parbar": @"",
                   @"Neariah": @"",
                   @"Yoel": @"- Joel",
                   @"Tiria": @"",
                   @"Coos": @"",
                   @"Dura": @"",
                   @"Maaseiah": @"",
                   @"Besor": @"",
                   @"Jeremai": @"",
                   @"Lucifer": @"",
                   @"Hor": @"-hagidgad",
                   @"Jabin": @"Jabneh whom God observes discerner the wise God discerns or intelligent he understands",
                   @"Ispah": @"",
                   @"Gibeon": @"",
                   @"Shamer": @"",
                   @"Goath": @"",
                   @"Shelomi": @"",
                   @"Azriel": @"help of God",
                   @"Joses": @"exalted",
                   @"Keiiah": @"",
                   @"Libnah": @"",
                   @"Abital": @"father of the dew father of the dew i.e. fresh",
                   @"Tappuah": @"",
                   @"Jathniel": @"whom God gives",
                   @"Syntyche": @"",
                   @"Alian": @"",
                   @"Nadab": @"Willing Liberal",
                   @"Ishma": @"distinction elevated",
                   @"Tubal": @"-Cain",
                   @"East": @"which is before or in front of a person",
                   @"Azzur": @"one who helps",
                   @"Elpaal": @"",
                   @"Kolaiah": @"",
                   @"Mibsam": @"",
                   @"Deuel": @"",
                   @"Bene": @"-jaakan",
                   @"Zacher": @"memento recollection commemoration",
                   @"Eliphalet": @"",
                   @"Huppim": @"",
                   @"Lemuel": @"",
                   @"Shimrath": @"",
                   @"Lilith": @"",
                   @"Neah": @"",
                   @"Anna": @"grace",
                   @"Mishmannah": @"",
                   @"Dodo": @"",
                   @"Achan": @"or Achar troubler",
                   @"Eltolad": @"",
                   @"Abiasaph": @"father of gathering i.e. gathered father of gathering the gatherer",
                   @"Elihoreph": @"",
                   @"Zophar": @"",
                   @"Mizpah": @"",
                   @"Benimi": @"",
                   @"Tekel": @"",
                   @"Jakeh": @"pious or hearkening",
                   @"Carmi": @"Vine-dresser or My Vineyard",
                   @"Trachonitis": @"",
                   @"Sharon": @"",
                   @"Sargon": @"",
                   @"Bathsheba": @"",
                   @"Hamul": @"",
                   @"Ikkesh": @"stubbord perverse subtle",
                   @"Ammi": @"my people",
                   @"Meah": @"",
                   @"Joed": @"",
                   @"Shelah": @"",
                   @"Anim": @"",
                   @"Tahrea": @"",
                   @"Jezreel": @"",
                   @"Jaresiah": @"whom YHWH nourishes",
                   @"Jah": @"abbreviation of YHWH",
                   @"Aholibamah": @"my tabernacle is exalted",
                   @"Drusilla": @"",
                   @"Paruah": @"",
                   @"Jabez": @"he makes sorrow or height sorrow",
                   @"Belteshazzar": @"",
                   @"Izehar": @"",
                   @"Ehud": @"",
                   @"Ibleam": @"",
                   @"Shelumiel": @"",
                   @"Calvary": @"",
                   @"Suah": @"",
                   @"Telharsa": @"",
                   @"Hanniel": @"",
                   @"Lior": @"",
                   @"Chesed": @"",
                   @"Hasrah": @"",
                   @"Tikvah": @"",
                   @"Gomorrah": @"",
                   @"Shamgar": @"",
                   @"Chesil": @"",
                   @"Jezebel": @"",
                   @"Dalmanutha": @"",
                   @"Meshillamith": @"",
                   @"Habakkuk": @"",
                   @"Johanan": @"whom YHWH graciously bestows gift or grace of God YHWH is or has been gracious.",
                   @"Lebbeus": @"",
                   @"Jehovah": @"-tsidkenu YHWH our righteousness",
                   @"Gaddiel": @"",
                   @"Barnabas": @"",
                   @"Epicurean": @"",
                   @"Habor": @"",
                   @"Haruphite": @"",
                   @"Ram": @"",
                   @"Talitha": @"-cumi",
                   @"Rohgah": @"",
                   @"Shem": @"",
                   @"Dekar": @"lance bearer perforation",
                   @"Vophsi": @"",
                   @"Maher": @"-shalal-hash-baz",
                   @"Jeush": @"",
                   @"Agagite": @"",
                   @"Assyria": @"",
                   @"Shilshah": @"",
                   @"Elm": @"strong",
                   @"Elim": @"",
                   @"Gittites": @"",
                   @"Boaz": @"",
                   @"Geber": @"",
                   @"Abigail": @"father i.e. source of joy",
                   @"Micaiah": @"",
                   @"Shaalbonite": @"",
                   @"Julius": @"",
                   @"Barak": @"",
                   @"Esau": @"",
                   @"Tabeal": @"",
                   @"Tiberias": @"",
                   @"Oshea": @"",
                   @"Helbah": @"Helbon",
                   @"Kirjath": @"-sepher",
                   @"Vashti": @"",
                   @"Hadadezer": @"",
                   @"Hoshama": @"",
                   @"Dalaiah": @"variant spelling of Delaiah",
                   @"Luhith": @"",
                   @"Ibsam": @"fragrant",
                   @"Jediel": @"",
                   @"Magdalene": @"",
                   @"Hazarmaveth": @"",
                   @"Shuni": @"",
                   @"Ithra": @"excellent",
                   @"Shaaraim": @"",
                   @"Pedahzur": @"",
                   @"Jesui": @"",
                   @"Michmash": @"",
                   @"Ulam": @"",
                   @"Gilalai": @"",
                   @"Zeredathah": @"",
                   @"Rhesa": @"",
                   @"Addi": @"ornament",
                   @"Elishaphat": @"",
                   @"Aduel": @"",
                   @"Jedeiah": @"",
                   @"Levi": @"joined to adhere adhesion",
                   @"Dinhabah": @"",
                   @"Phlegon": @"",
                   @"Cana": @"",
                   @"Kirjathaim": @"",
                   @"Reelaiah": @"",
                   @"Geder": @"",
                   @"Tabelel": @"God is good",
                   @"Aphik": @"",
                   @"Seleucia": @"",
                   @"Casiphia": @"silversmith",
                   @"Zavan": @"",
                   @"Michtam": @"",
                   @"Ziz": @"",
                   @"Nahum": @"",
                   @"Huz": @"",
                   @"Pekah": @"",
                   @"Aggaba": @"(variant of Hagabah)",
                   @"Mene": @"",
                   @"Enam": @"",
                   @"Ahaziah": @"",
                   @"Rehoboam": @"",
                   @"So": @"",
                   @"Amoz": @"strong robust",
                   @"Elkanah": @"",
                   @"Cush": @"Cushan",
                   @"Ahuzzah": @"",
                   @"Huldah": @"",
                   @"Hillel": @"",
                   @"Baal": @"-zephon",
                   @"Galilee": @"",
                   @"Nehum": @"",
                   @"Delaiah": @"YHWH is deliverer",
                   @"Avim": @"",
                   @"Tarah": @"",
                   @"Abijam": @"father of the sea i.e. seaman Abijah or Abijam: @my father is YHWH",
                   @"Jordan": @"Some translate it as the descender from the Semitic yrd to descend",
                   @"Maaz": @"",
                   @"Addin": @"",
                   @"Jerusalem": @"the habitation of peace",
                   @"Achmetha": @"",
                   @"Peulthai": @"",
                   @"Nebuzar": @"-adan",
                   @"Koa": @"",
                   @"Magpiash": @"",
                   @"Balak": @"",
                   @"Jerubbesheth": @"",
                   @"Shicron": @"",
                   @"Harod": @"",
                   @"Irad": @"wild ass heap of empire dragon fleet",
                   @"Ahijah": @"brother of the Lord",
                   @"Iturea": @"",
                   @"Betonim": @"",
                   @"Gimzo": @"fertile in sycamores a place fertile in sycamores",
                   @"Ishua": @"",
                   @"Jahleel": @"God waits or God does grievously afflict",
                   @"Philemon": @"",
                   @"Ittai": @"with me plowman living",
                   @"Babylon": @"Gate Of The Deity anointment or consecration or confusion or mixing",
                   @"Gaius": @"",
                   @"Felix": @"happy happy prosperous",
                   @"Kelitah": @"",
                   @"Jediael": @"",
                   @"Ibneiah": @"God builds YHWH does build",
                   @"India": @"",
                   @"Gemariah": @"",
                   @"Hananiah": @"",
                   @"Mehuman": @"",
                   @"Seled": @"",
                   @"Tertullus": @"",
                   @"Tiglath": @"-Pileser",
                   @"Nobah": @"",
                   @"Admatha": @"",
                   @"Mahavites": @"",
                   @"Bochim": @"",
                   @"Janna": @"",
                   @"Zorites": @"",
                   @"Imrah": @"a rebel stubborn height of YHWH",
                   @"Sodom": @"",
                   @"Decapolis": @"",
                   @"Baruch": @"",
                   @"Parshandatha": @"",
                   @"Pur": @"",
                   @"Hen": @"",
                   @"Shecaniah": @"Yah has taken up His abode",
                   @"Ishi": @"saving my help saving",
                   @"Rezeph": @"",
                   @"Dalphon": @"",
                   @"Zilpah": @"",
                   @"Jagur": @"",
                   @"Abdiel": @"servant of God",
                   @"Jareb": @"",
                   @"Hodesh": @"",
                   @"Zebaim": @"",
                   @"Zibia": @"",
                   @"Ezekiel": @"God Strengthens",
                   @"Cuth": @"",
                   @"Pharpar": @"",
                   @"Shebaniah": @"",
                   @"Aram": @"high",
                   @"Helez": @"",
                   @"Eshtaol": @"",
                   @"Tibbath": @"",
                   @"Jonan": @"",
                   @"Gideoni": @"",
                   @"Manoah": @"",
                   @"Rephael": @"",
                   @"Anani": @"",
                   @"Asnapper": @"",
                   @"Galeed": @"",
                   @"Ahasbai": @"",
                   @"Vashni": @"",
                   @"Abiah": @"",
                   @"Ben": @"-ammi",
                   @"Antipatris": @"for his father",
                   @"Bithron": @"",
                   @"Tahapenes": @"",
                   @"Jesher": @"",
                   @"Emmanuel": @"",
                   @"Giloh": @"",
                   @"Keren": @"-happuch",
                   @"Merodach": @"-baladan",
                   @"Non": @"",
                   @"Jambres": @"",
                   @"Bul": @"",
                   @"Gaza": @"",
                   @"Adaiah": @"",
                   @"Ramoth": @"",
                   @"Gibbar": @"",
                   @"Chilion": @"",
                   @"Barabbas": @"",
                   @"Elath": @"",
                   @"Shaalbim": @"",
                   @"Hallohesh": @"",
                   @"Gad": @"He has granted me good fortune",
                   @"Epher": @"",
                   @"Pathros": @"",
                   @"Pergamos": @"",
                   @"Jaanai": @"whom YHWH answers mourner YHWH answers",
                   @"Jahmai": @"YHWH protects",
                   @"Jahzeel": @"God apportions or distributes",
                   @"Jahzerah": @"YHWH protects or may he lead back",
                   @"Elishama": @"",
                   @"Julia": @"",
                   @"Jaalam": @"whom God hides concealer he will be hid",
                   @"Pashur": @"",
                   @"Aphekah": @"",
                   @"Pinon": @"",
                   @"Artaxerxes": @"the great warrior",
                   @"Ezer": @"",
                   @"Ai": @"or Hai heap of ruins",
                   @"Merab": @"",
                   @"Martha": @"who becomes bitter provoking a lady",
                   @"Shear": @"-jashub",
                   @"Idalah": @"",
                   @"Telem": @"",
                   @"Molech": @"",
                   @"Evi": @"",
                   @"Heth": @"",
                   @"Magbish": @"",
                   @"Shelesh": @"",
                   @"Zalmon": @"shady",
                   @"Azaliah": @"Yahweh has reserved",
                   @"Ezel": @"",
                   @"Adoraim": @"",
                   @"Eleasah": @"whom God made",
                   @"Mazzaroth": @"",
                   @"Jehoshua": @"",
                   @"Hilen": @"",
                   @"Micha": @"",
                   @"Nehushta": @"",
                   @"Christ": @"anointed The Anointed One",
                   @"Nachor": @"",
                   @"Nephthalim": @"",
                   @"Elienai": @"",
                   @"Hobab": @"",
                   @"Michaiah": @"",
                   @"Agur": @"",
                   @"Hiram": @"",
                   @"Gerar": @"",
                   @"Shebam": @"",
                   @"Shubael": @"",
                   @"Apocrypha": @"concealed hidden",
                   @"Resen": @"",
                   @"Reumah": @"",
                   @"Keilah": @"",
                   @"Hachmoni": @"",
                   @"Salem": @"",
                   @"Ami": @"",
                   @"Dorcas": @"",
                   @"Assir": @"captive",
                   @"Ahira": @"brother of evil i.e. unlucky",
                   @"Aroer": @"",
                   @"Zerahiah": @"",
                   @"Isshiah": @"there is God",
                   @"Jericho": @"",
                   @"Andrew": @"manly",
                   @"Hachaliah": @"",
                   @"Chisloth": @"-tabor",
                   @"Emmor": @"",
                   @"Togarmah": @"which is all bone",
                   @"Jephthah": @"",
                   @"Azaziah": @"",
                   @"Ithream": @"populous remnant abundance of the people",
                   @"Lucas": @"",
                   @"Mareshah": @"",
                   @"Nun": @"",
                   @"Riphath": @"",
                   @"Korah": @"",
                   @"Potiphar": @"",
                   @"Amasai": @"",
                   @"Adin": @"Adina dainty delicate",
                   @"Sheariah": @"",
                   @"Hezrai": @"",
                   @"Hashabiah": @"",
                   @"Maale": @"-akrabbim",
                   @"Lmri": @"eloquent",
                   @"Jonadab": @"",
                   @"Abinoam": @"",
                   @"Jazeel": @"",
                   @"Ahio": @"",
                   @"Hauran": @"",
                   @"Tob": @"-adonijah",
                   @"Shabbethai": @"",
                   @"Jedidiah": @"beloved of YHWH",
                   @"Ophni": @"",
                   @"Azazel": @"",
                   @"Zoheleth": @"",
                   @"Jearim": @"",
                   @"Philetus": @"",
                   @"Mijamin": @"From The Right Side",
                   @"Ragau": @"",
                   @"Nathanael": @"",
                   @"Peter": @"a rock or stone",
                   @"Mattatha": @"An ancestor of Iesus",
                   @"Megiddon": @"",
                   @"Toby": @"Tobias the Lord is kind",
                   @"Jabneel": @"building of God built by God",
                   @"Nachon": @"",
                   @"Geba": @"",
                   @"Havoth": @"-jair",
                   @"Mejarkon": @"",
                   @"Hazezon": @"-tamar",
                   @"Ammihud": @"people of praise",
                   @"Eden": @"",
                   @"Shashak": @"",
                   @"Zibiah": @"",
                   @"Thessalonica": @"",
                   @"Kenizzites": @"",
                   @"Ashnah": @"",
                   @"Diblah": @"",
                   @"Nagge": @"",
                   @"Hashub": @"",
                   @"Jerimoth": @"",
                   @"Almon": @"-diblathaim",
                   @"Jehozabad": @"YHWH-given whom YHWH gave",
                   @"Bilshan": @"",
                   @"Agar": @"",
                   @"Calneh": @"",
                   @"Horonaim": @"",
                   @"Jeremoth": @"",
                   @"Leummim": @"",
                   @"Lamech": @"",
                   @"Ashan": @"smoke",
                   @"Aman": @"",
                   @"Neginoth": @"",
                   @"Josiah": @"",
                   @"Hotham": @"",
                   @"Gomer": @"",
                   @"Zebina": @"",
                   @"Jesimiel": @"",
                   @"Jeziah": @"whom YHWH expiates",
                   @"Lahmam": @"",
                   @"Antichrist": @"",
                   @"Zanoah": @"marsh",
                   @"Jashub": @"",
                   @"Izri": @"creative former",
                   @"Jehoshaphat": @"whom YHWH judges YHWH-judged",
                   @"Azotus": @"",
                   @"Jadau": @"favorite or friend",
                   @"Lahmi": @"",
                   @"Ashdod": @"",
                   @"Calcol": @"",
                   @"Janum": @"",
                   @"Sergius": @"",
                   @"Enan": @"",
                   @"Madai": @"",
                   @"Susanna": @"",
                   @"Chelubai": @"",
                   @"Aesora": @"",
                   @"Abagtha": @"God-given etymology doubtful",
                   @"Sibbechai": @"",
                   @"Heleph": @"",
                   @"Hezekiah": @"",
                   @"Gazabar": @"",
                   @"Derbe": @"",
                   @"Hazo": @"",
                   @"Gera": @"",
                   @"Ahiram": @"",
                   @"Shalman": @"",
                   @"Nephthoah": @"",
                   @"Cabbon": @"",
                   @"Immanuel": @"God is with is Hebrew c. 8th century",
                   @"Hadrach": @"",
                   @"Mattan": @"Mattana Mattenai gifts rains",
                   @"Elnaam": @"",
                   @"Achaz": @"",
                   @"Aniam": @"",
                   @"Raamiah": @"",
                   @"Hupham": @"",
                   @"Zemira": @"",
                   @"Trophimus": @"",
                   @"Igeal": @"",
                   @"Eliasaph": @"",
                   @"Hai": @"same as Ai = heap of ruins",
                   @"Epaphras": @"",
                   @"Ater": @"shut up",
                   @"Sabeans": @"",
                   @"Tabor": @"",
                   @"Elzaphan": @"",
                   @"Barzillai": @"",
                   @"Melech": @"",
                   @"Zebulun": @"dwelling habitation",
                   @"Zalmunna": @"",
                   @"Jeshurun": @"",
                   @"Moreh": @"",
                   @"Mordecai": @"dedicated to Mars a little man or bitter bruising",
                   @"Jashem": @"",
                   @"Zared": @"",
                   @"Geliloth": @"",
                   @"Shammah": @"",
                   @"Medeba": @"",
                   @"Naasson": @"enchanter",
                   @"Hebrews": @"",
                   @"Jekabzeel": @"",
                   @"Akkad": @"",
                   @"Magdiel": @"",
                   @"Atarah": @"a crown",
                   @"Og": @"",
                   @"Heli": @"",
                   @"Raddai": @"",
                   @"Amalek": @"",
                   @"Shamhuth": @"",
                   @"Eve": @"",
                   @"Harhur": @"",
                   @"Apollonius": @"",
                   @"Clauda": @"",
                   @"Beth": @"-zur",
                   @"Amittai": @"",
                   @"Zacchaeus": @"pure clean just",
                   @"Jozachar": @"",
                   @"Saruch": @"",
                   @"Mahazioth": @"",
                   @"Gittaim": @"",
                   @"Tetrarch": @"",
                   @"Zerubbabel": @"",
                   @"Beelzebub": @"",
                   @"Hymeneus": @"",
                   @"Shoham": @"",
                   @"Miamin": @"",
                   @"Aner": @"",
                   @"Jehaziel": @"",
                   @"Shalem": @"",
                   @"Dedan": @"low their friendship",
                   @"Phygellus": @"",
                   @"Oboth": @"",
                   @"Ahiman": @"brother of the right hand",
                   @"Melea": @"",
                   @"Sadducees": @"",
                   @"Hamath": @"-zobah",
                   @"Berachah": @"",
                   @"Miletum": @"",
                   @"Babel": @"confusion mixture confusion gate of God",
                   @"Jebus": @"",
                   @"Myra": @"",
                   @"Dishan": @"",
                   @"Zidkijah": @"",
                   @"Amariah": @"the Lord says i.e. promises",
                   @"Bezaleel": @"",
                   @"Candace": @"",
                   @"Asahel": @"made by God",
                   @"Euodias": @"",
                   @"Gath": @"-rimmon",
                   @"Tabrimon": @"",
                   @"Abishur": @"father of the wall father of the wall i.e. mason",
                   @"Ararat": @"",
                   @"Harsha": @"",
                   @"Zobah": @"",
                   @"Hadlai": @"",
                   @"Anammelech": @"",
                   @"Binnui": @"",
                   @"Elzabad": @"",
                   @"Anub": @"",
                   @"Zebedee": @"my gift",
                   @"Jehoiachin": @"",
                   @"Jabesh": @"a dry place dry",
                   @"Kenites": @"",
                   @"Jaasau": @"whom YHWH made fabricator YHWH makes",
                   @"Jahaziah": @"YHWH reveals YHWH sees",
                   @"Ishuah": @"Isuah equal self-satisfied",
                   @"Nekoda": @"",
                   @"Jattir": @"",
                   @"Molid": @"begetter",
                   @"Kir": @"-haraseth",
                   @"Rogelim": @"",
                   @"Kartah": @"",
                   @"Zair": @"little small",
                   @"Shoshannim": @"",
                   @"Nahaliel": @"",
                   @"Zaza": @"",
                   @"Sharai": @"",
                   @"Agag": @"",
                   @"Amashai": @"",
                   @"Othni": @"",
                   @"Hazor": @"",
                   @"Agrippa": @"",
                   @"Zichri": @"",
                   @"Belah": @"",
                   @"Jed": @"",
                   @"Sin": @"",
                   @"Zepho": @"",
                   @"Earnest": @"pledge",
                   @"Juttah": @"",
                   @"Baanah": @"son of grief",
                   @"Shomer": @"",
                   @"Bethanath": @"",
                   @"Tohu": @"",
                   @"Hophra": @"",
                   @"Achaicus": @"belonging to Achaia",
                   @"Mikloth": @"",
                   @"Madian": @"",
                   @"Parnach": @"",
                   @"Ahimelech": @"brother of the king",
                   @"Ziphah": @"",
                   @"Naboth": @"",
                   @"Ahian": @"",
                   @"Bichri": @"",
                   @"Salah": @"",
                   @"Ahitub": @"brother of goodness",
                   @"Jerusha": @"",
                   @"Ono": @"",
                   @"Zemaraim": @"",
                   @"Chebar": @"",
                   @"Penuel": @"",
                   @"Isaiah": @"Gods salvation YHWH is helper salvation is of the Lord",
                   @"Gemalli": @"",
                   @"Memucan": @"",
                   @"Zechariah": @"",
                   @"Bekah": @"",
                   @"Sur": @"",
                   @"Judaea": @"",
                   @"Kibzaim": @"",
                   @"Tadmor": @"",
                   @"Zebah": @"",
                   @"Elisha": @"",
                   @"Ethiopia": @"",
                   @"Shoa": @"",
                   @"Joash": @"",
                   @"Ibhar": @"he will choose chooser God does choose",
                   @"Gershon": @"Expelled One or Exiled One.",
                   @"Ziha": @"",
                   @"Raham": @"",
                   @"Josabad": @"",
                   @"Othniel": @"",
                   @"Shapham": @"",
                   @"Hachilah": @"",
                   @"Harbonah": @"",
                   @"Ir": @"-shemesh",
                   @"Jorim": @"",
                   @"Hilkiah": @"",
                   @"Iscah": @"who looks",
                   @"Achaia": @"trouble",
                   @"Chemosh": @"",
                   @"Sansannah": @"",
                   @"Sud": @"",
                   @"Rabshakeh": @"",
                   @"Sethur": @"",
                   @"Meribaal": @"",
                   @"Haahashtari": @"",
                   @"Bukkiah": @"",
                   @"Gazez": @"",
                   @"Beerelim": @"",
                   @"Eliel": @"",
                   @"Puah": @"",
                   @"Jarah": @"",
                   @"Horonites": @"",
                   @"Timnah": @"",
                   @"Hali": @"",
                   @"Kadmonites": @"",
                   @"Accad": @"",
                   @"Jamin": @"Right Hand",
                   @"Bebai": @"",
                   @"Shema": @"",
                   @"Melita": @"",
                   @"Arphaxad": @"",
                   @"Ethni": @"",
                   @"Moriah": @"",
                   @"Azrael": @"",
                   @"Adah": @"ornament ornament beauty",
                   @"Nazarite": @"",
                   @"Cheran": @"",
                   @"Silas": @"",
                   @"Adithaim": @"double ornament",
                   @"Senir": @"",
                   @"Attai": @"",
                   @"Phylacteries": @"",
                   @"Shamsherai": @"",
                   @"Elealeh": @"",
                   @"Bohan": @"thumb or big toe",
                   @"Berodach": @"-baladan",
                   @"Josiphiah": @"",
                   @"Kedemoth": @"",
                   @"Abihu": @"he (God) is my father father of Him i.e. worshipper of God",
                   @"Bealiah": @"",
                   @"Bethany": @"house of dates or house of misery House of Misery",
                   @"Zimran": @"",
                   @"Bithiah": @"",
                   @"Barachel": @"",
                   @"Kedemah": @"",
                   @"Admin": @"",
                   @"Philistines": @"",
                   @"Patara": @"",
                   @"Shamariah": @"",
                   @"Stephanas": @"",
                   @"Shimshai": @"",
                   @"Jorah": @"",
                   @"Jehoaddan": @"",
                   @"Bildad": @"",
                   @"Piram": @"",
                   @"Shamir": @"",
                   @"Sanhedrin": @"",
                   @"Oren": @"",
                   @"Nymphas": @"",
                   @"Bunah": @"",
                   @"Aenon": @"(or Ænon)",
                   @"Neapolis": @"",
                   @"Meshach": @"",
                   @"Gether": @"",
                   @"Abimael": @"father of Mael God is a father",
                   @"Bartimeus": @"",
                   @"Hizkijah": @"",
                   @"Mahaleleel": @"",
                   @"Ahiam": @"",
                   @"Samos": @"",
                   @"Tharah": @"",
                   @"Shemida": @"",
                   @"Betah": @"",
                   @"Rehob": @"",
                   @"Massah": @"",
                   @"Nebo": @"",
                   @"Cnidus": @"",
                   @"Hadad": @"",
                   @"Jehozadak": @"YHWH-justified YHWH justifies",
                   @"Oded": @"",
                   @"Mahanehdan": @"",
                   @"Rhodes": @"",
                   @"Sheber": @"",
                   @"Jasiel": @"whom God made",
                   @"Sinai": @"",
                   @"Hushathite": @"",
                   @"Anab": @"grape-town",
                   @"Holon": @"",
                   @"Zion": @"",
                   @"Phebe": @"",
                   @"Potipherah": @"",
                   @"Meshezaheel": @"",
                   @"Ibnijah": @"whom YHWH will build up God builds YHWH is builder",
                   @"Ziphron": @"",
                   @"Delilah": @"Samsons mistress languishing",
                   @"Dophkah": @"",
                   @"Ishmaiah": @"may God hear YHWH hears",
                   @"Thuhash": @"",
                   @"Neri": @"",
                   @"Laodicea": @"",
                   @"Chushan": @"-rishathaim",
                   @"Seir": @"",
                   @"Jedidah": @"",
                   @"Minni": @"",
                   @"Jehonadab": @"",
                   @"Mezahab": @"",
                   @"Samlah": @"",
                   @"Hoshaiah": @"",
                   @"Bamah": @"",
                   @"Elizabeth": @"",
                   @"Succoth": @"-benoth",
                   @"Gaal": @"",
                   @"Mara": @"Marah sad bitter",
                   @"Ezbon": @"",
                   @"Becher": @"",
                   @"Ammiel": @"",
                   @"Shunem": @"",
                   @"Ozni": @"",
                   @"Almodad": @"measure",
                   @"Moserah": @"",
                   @"Hephzibah": @"",
                   @"Er": @"",
                   @"Phalti": @"Palti",
                   @"Camon": @"derivation uncertain",
                   @"Zuar": @"",
                   @"Maadai": @"",
                   @"Noah": @"",
                   @"Jehdeiah": @"",
                   @"Zacharias": @"the Lord has remembered Greek form of Zechariah",
                   @"Mattaniah": @"gift of YHWH Original name of Zedekiah king of Judah",
                   @"Bunni": @"",
                   @"Tophet": @"",
                   @"Pilate": @"",
                   @"Beneberak": @"",
                   @"Meribah": @"",
                   @"Alleluia": @"praise ye YHWH",
                   @"Sebia": @"",
                   @"Ellasar": @"",
                   @"Adiel": @"",
                   @"Cappadocia": @"",
                   @"Abana": @"perennial stony",
                   @"Baladan": @"",
                   @"Uzzen": @"-sherah",
                   @"Amad": @"",
                   @"Hadar": @"",
                   @"Abba": @"father",
                   @"Elisheba": @"",
                   @"Zillah": @"",
                   @"Memphis": @"",
                   @"Muppim": @"",
                   @"Cainan": @"Kenan acquisition",
                   @"Jochebed": @"",
                   @"Eldad": @"",
                   @"Tebeth": @"",
                   @"Nimrah": @"",
                   @"Menahem": @"",
                   @"Izziah": @"Jeziah YHWH exalts",
                   @"Belshazzar": @"",
                   @"Nicolaitanes": @"",
                   @"Benjamin": @"Son of His right hand",
                   @"Lehabim": @"",
                   @"Joha": @"Juha",
                   @"Imla": @"whom God will fill up replenisher",
                   @"Zidon": @"",
                   @"Jetur": @"",
                   @"Dannah": @"",
                   @"Eluzai": @"",
                   @"Gerizim": @"",
                   @"Gethsemane": @"",
                   @"Lecah": @"progress",
                   @"Bethuel": @"",
                   @"Adonikam": @"",
                   @"Arba": @"city of the four",
                   @"Jerijah": @"",
                   @"Abiezer": @"(or Abieezer) father of help helpful",
                   @"Coz": @"",
                   @"Hodaiah": @"",
                   @"Elead": @"",
                   @"Tirshatha": @"",
                   @"Izrahiah": @"may God shine forth YHWH is appearing does arise",
                   @"Karnaim": @"",
                   @"Solomon": @"",
                   @"Esrom": @"",
                   @"Hophin": @"",
                   @"Beeri": @"",
                   @"Zizah": @"",
                   @"Raca": @"",
                   @"Urbane": @"",
                   @"Philip": @"lover of horses",
                   @"Chedorlaomer": @"",
                   @"Zerah": @"",
                   @"Sabaoth": @"",
                   @"Jothath": @"",
                   @"Anathoth": @"",
                   @"Abiel": @"father (i.e. possessor) of God = pious",
                   @"Ahuzam": @"",
                   @"Aran": @"wild goat",
                   @"Isui": @"",
                   @"Kirioth": @"",
                   @"Damaris": @"",
                   @"Sitnah": @"",
                   @"Hukkok": @"",
                   @"Ishtob": @"",
                   @"Jehaleleel": @"",
                   @"Seraphim": @"",
                   @"Pelaliah": @"",
                   @"Corinth": @"",
                   @"Herod": @"",
                   @"Zattu": @"",
                   @"Milcah": @"",
                   @"Deborah": @"",
                   @"Jucal": @"",
                   @"Tebaliah": @"",
                   @"Peruda": @"",
                   @"Ishiah": @"YHWH exists forgiveth",
                   @"Roman": @"",
                   @"Meraioth": @"",
                   @"Rachal": @"",
                   @"Azzan": @"",
                   @"Shochoh": @"",
                   @"Mecherath": @"",
                   @"Quicksands": @"The Greater Sytis ...fearing lest they should be cast upon the Syrtis...",
                   @"Zina": @"",
                   @"Balaam": @"a pilgrim devouring lord of the people",
                   @"Syene": @"",
                   @"Athaiah": @"meaning obscure",
                   @"Rabmag": @"",
                   @"Abijah": @"father (i.e. possessor or worshipper) of YHWH",
                   @"Salome": @"",
                   @"Palal": @"",
                   @"Zareth": @"-shahar",
                   @"Nathan": @"-melech",
                   @"Zoan": @"",
                   @"Eshtemoa": @"",
                   @"Norah": @"",
                   @"Elnathan": @"",
                   @"Jairus": @"He will enlighten or diffuse light",
                   @"Tryphosa": @"",
                   @"Akkub": @"",
                   @"Debir": @"speaker",
                   @"Ishod": @"famed man of honor man of splendor",
                   @"Apollonia": @"",
                   @"Benhail": @"",
                   @"Ibzan": @"father of a target father of coldness splendid active",
                   @"Gergesenes": @"",
                   @"Neziah": @"",
                   @"Lakum": @"fortification",
                   @"Elah": @"",
                   @"Prisca": @"",
                   @"Micah": @"",
                   @"Allon": @"-bachuth",
                   @"Raphah": @"",
                   @"Noe": @"",
                   @"Ahihud": @"",
                   @"Gehazi": @"",
                   @"Chelub": @"",
                   @"Pharez": @"",
                   @"Shethar": @"-boznai",
                   @"Eldaah": @"",
                   @"Simon": @"",
                   @"Rinnah": @"",
                   @"Chenaniah": @"",
                   @"Mattathias": @"",
                   @"Elioenai": @"",
                   @"Abubus": @"",
                   @"Paulus": @"",
                   @"Hiddai": @"",
                   @"Medad": @"",
                   @"Jephunneh": @"",
                   @"Illyricum": @"",
                   @"Hezron": @"",
                   @"Italy": @"",
                   @"Rekem": @"",
                   @"Theophilus": @"",
                   @"Hermon": @"",
                   @"Jokmeam": @"",
                   @"Idbash": @"honey-sweet corpulent",
                   @"Zia": @"",
                   @"Hadassah": @"",
                   @"Zereth": @"",
                   @"Tel": @"-melah",
                   @"Lud": @"",
                   @"Urim": @"",
                   @"Jonathan": @"",
                   @"Merari": @"",
                   @"Asiel": @"created by God",
                   @"Henoch": @"",
                   @"Charchemish": @"",
                   @"Lebanon": @"",
                   @"Malluch": @"",
                   @"Sharezer": @"",
                   @"Agabus": @"",
                   @"Tabering": @"to beat with loud strokes",
                   @"Tartan": @"",
                   @"Askelon": @"",
                   @"Eunice": @"",
                   @"Zaham": @"fatness",
                   @"Mesopotamia": @"",
                   @"Ajalon": @"",
                   @"Nepheg": @"",
                   @"Ziphites": @"",
                   @"Jehoahaz": @"",
                   @"Uzziah": @"",
                   @"James": @"the Greek form of Jacob supplanter (to take the place of another as through force scheming strategy or the like)",
                   @"Dilean": @"",
                   @"Ahava": @"",
                   @"Iddo": @"Gods friend affectionate festal his power",
                   @"Joachim": @"",
                   @"Tahath": @"",
                   @"Mizzah": @"",
                   @"Milalai": @"",
                   @"Zelek": @"",
                   @"Adriel": @"God is helper",
                   @"Kibroth": @"-hattaavah",
                   @"Rehoboth": @"",
                   @"Eliashib": @"God Will Restore. Whom God Restore or Whom God Leads Back Again. God Restores.",
                   @"Jaakobah": @"supplanter deceiver the heel",
                   @"Reaiah": @"",
                   @"Lazarus": @"",
                   @"Labana": @"",
                   @"Hadadrimmon": @"",
                   @"Secacah": @"",
                   @"Jehezekel": @"God strengthens",
                   @"Eliakim": @"",
                   @"Hazar": @"-susah or -susim",
                   @"Pul": @"",
                   @"Chephirah": @"",
                   @"Jehoash": @"YHWH-given",
                   @"Shamed": @"",
                   @"Jeremiah": @"raised up or appointed by YHWH whom YHWH has appointed",
                   @"Bezai": @"",
                   @"Eran": @"",
                   @"Timeus": @"",
                   @"Shaul": @"Asked of God",
                   @"Zethar": @"",
                   @"Hagar": @"",
                   @"Diblath": @"",
                   @"Hammoleketh": @"the queen",
                   @"Emims": @"",
                   @"Ebronah": @"",
                   @"Meres": @"",
                   @"Haggiah": @"",
                   @"Shaashgaz": @"",
                   @"Adramyttium": @"",
                   @"Tamar": @"",
                   @"Zered": @"",
                   @"Meshelemiah": @"",
                   @"Eliathah": @"",
                   @"Tebah": @"",
                   @"Belial": @"",
                   @"Abilene": @"land of meadows",
                   @"Ashriel": @"",
                   @"Peleg": @"",
                   @"Adna": @"",
                   @"Necho": @"",
                   @"Heshmon": @"",
                   @"Kenah": @"",
                   @"Habazziniah": @"",
                   @"Kezia": @"",
                   @"Ije": @"-abarim",
                   @"Goliath": @"",
                   @"Nicanor": @"",
                   @"Haggith": @"",
                   @"Ishmael": @"God that hears hears",
                   @"Cyrene": @"",
                   @"Tirras": @"desire",
                   @"Galatia": @"",
                   @"Pharaoh": @"",
                   @"Jahdai": @"guide or he directs",
                   @"Jimnah": @"",
                   @"Haggai": @"",
                   @"Jeberechiah": @"whom YHWH blesses",
                   @"Shimma": @"",
                   @"Priscilla": @"",
                   @"Carmel": @"",
                   @"Uzal": @"",
                   @"Ham": @"",
                   @"Hashem": @"",
                   @"Abez": @"lofty",
                   @"Jeheiel": @"",
                   @"Hodaviah": @"",
                   @"Cilicia": @"",
                   @"Nephusim": @"",
                   @"Uzai": @"",
                   @"Zuriel": @"",
                   @"Sibmah": @"",
                   @"Maon": @"",
                   @"Vajezatha": @"",
                   @"Ardon": @"",
                   @"Earing": @"ploughing plough or till",
                   @"Dizahab": @"",
                   @"Sophereth": @"",
                   @"Jalon": @"abiding lodger",
                   @"Hapharaim": @"",
                   @"Japheth": @"",
                   @"Elkoshite": @"",
                   @"Rakkath": @"",
                   @"Padan": @"-aram",
                   @"Horem": @"",
                   @"Siloa": @"",
                   @"Horims": @"",
                   @"Herodion": @"",
                   @"Regem": @"",
                   @"Dinah": @"",
                   @"Thahash": @"",
                   @"Ahasuerus": @"",
                   @"Joahaz": @"",
                   @"Adummim": @"",
                   @"Tertius": @"",
                   @"Matthan": @"gift",
                   @"Poratha": @"",
                   @"Ain": @"spring well",
                   @"Asshurim": @"possibly peasants",
                   @"Shebuel": @"",
                   @"Jason": @"He that will cure one who will heal",
                   @"Jachin": @"he shall establish he does establish or founding established",
                   @"Hamonah": @"",
                   @"Gabbai": @"",
                   @"Omar": @"",
                   @"Timothy": @"",
                   @"Amplias": @"large",
                   @"Azor": @"",
                   @"Chimham": @"",
                   @"Onan": @"",
                   @"Dodai": @"",
                   @"Jehohanan": @"whom YHWH gave a name of which John is the contraction.",
                   @"Halak": @"",
                   @"Claudia": @"Claudius lame",
                   @"Parosh": @"",
                   @"Cleophas": @"",
                   @"Zererath": @"",
                   @"Sia": @"",
                   @"Apollos": @"",
                   @"Shemer": @"",
                   @"Mellicu": @"",
                   @"Shobal": @"",
                   @"Baaseiah": @"YHWH is bold",
                   @"Shepho": @"",
                   @"Tola": @"",
                   @"Chenani": @"",
                   @"Rosh": @"",
                   @"Salathiel": @"",
                   @"Rehabiah": @"",
                   @"Cozbi": @"",
                   @"Yakman": @"",
                   @"Zabad": @"gift",
                   @"Achzib": @"lying false",
                   @"Bedan": @"",
                   @"Ishbi": @"-benob man sitting in Nob dweller on the mount he that predicts",
                   @"Ishmerai": @"God guards God keeps",
                   @"Aceldama": @"field of blood",
                   @"Joanna": @"whom YHWH has graciously given grace or gift of God",
                   @"Zophah": @"",
                   @"Zephi": @"",
                   @"Hasadiah": @"",
                   @"Gebim": @"",
                   @"Havilah": @"",
                   @"Nason": @"",
                   @"Nephishesim": @"",
                   @"Shimri": @"",
                   @"Ichabod": @"inglorious the glory is not where is the glory inglorious",
                   @"Barjona": @"",
                   @"Zeresh": @"",
                   @"Shihor": @"-libnah",
                   @"Shophach": @"",
                   @"Zalmonah": @"shady",
                   @"Zif": @"",
                   @"Nereus": @"",
                   @"Assos": @"approaching",
                   @"Mammon": @"",
                   @"Ataroth": @"-addar",
                   @"Aretas": @"",
                   @"Eladah": @"",
                   @"Elishah": @"",
                   @"Nebaioth": @"",
                   @"Henadad": @"",
                   @"Esh": @"-ban",
                   @"Jezaniah": @"whom YHWH hears",
                   @"Nethaniah": @"",
                   @"Sidon": @"",
                   @"Sherah": @"",
                   @"Tarea": @"",
                   @"Hathath": @"",
                   @"Lael": @"",
                   @"Rei": @"",
                   @"Eliphaz": @"",
                   @"Anem": @"",
                   @"Adlai": @"",
                   @"Shashai": @"",
                   @"Shimrith": @"",
                   @"Adnah": @"",
                   @"Esar": @"-haddon",
                   @"Ozem": @"",
                   @"Araunah": @"ark",
                   @"Keziz": @"",
                   @"Gallio": @"",
                   @"Achim": @"",
                   @"Aphek": @"strength",
                   @"Ziphion": @"",
                   @"Shiloh": @"(name of a city)",
                   @"Makas": @"",
                   @"Jeriel": @"",
                   @"Cedron": @"",
                   @"Jaala": @"ascending a little doe or goat wild goat",
                   @"Lebaoth": @"",
                   @"Abram": @"a high father",
                   @"Jaasu": @"created",
                   @"Quaternion": @"a guard of four soldiers ...and delivered him to four quaternions of soldiers to guard him...",
                   @"Shur": @"",
                   @"Alpheus": @"",
                   @"Beulah": @"",
                   @"Garmites": @"",
                   @"Hammon": @"",
                   @"Zipporah": @"",
                   @"Keros": @"",
                   @"Crescens": @"",
                   @"Hazelelponi": @"",
                   @"Maleleel": @"",
                   @"Huzoth": @"",
                   @"Jonah": @"",
                   @"Joab": @"",
                   @"Reba": @"",
                   @"Shimon": @"",
                   @"Sodi": @"",
                   @"Chaldea": @"",
                   @"Abi": @"-albon father of strength i.e. valiant also called ABIEL",
                   @"Shilom": @"",
                   @"Epenetus": @"",
                   @"Caesar": @"",
                   @"Ornan": @"",
                   @"Elon": @"-beth-hanan",
                   @"Pallu": @"Separated, Distinguished",
                   @"Tychicus": @"",
                   @"Bethesda": @"",
                   @"Calah": @"",
                   @"Sihor": @"",
                   @"Achbor": @"mouse",
                   @"Cosam": @"",
                   @"Didymus": @"",
                   @"Sallai": @"",
                   @"Methusael": @"",
                   @"Jushabhesed": @"",
                   @"Riblah": @"",
                   @"Tahan": @"",
                   @"Kitron": @"",
                   @"Rahab": @"",
                   @"Admah": @"",
                   @"Dor": @"",
                   @"Dibon": @"-gad",
                   @"Dedanim": @"",
                   @"Chesulloth": @"",
                   @"Dathan": @"belonging to law",
                   @"Jehudijah": @"",
                   @"Shephuphan": @"",
                   @"Azrikam": @"",
                   @"Ishbosheth": @"a man of shame",
                   @"Nethinims": @"",
                   @"Zephon": @"",
                   @"Rechab": @"",
                   @"Eltekeh": @"",
                   @"Abdeel": @"servant of God",
                   @"Amram": @"an exalted people",
                   @"Athaliah": @"",
                   @"Japho": @"",
                   @"Mephaath": @"",
                   @"Baara": @"",
                   @"Beten": @"",
                   @"Chloe": @"",
                   @"Goshen": @"",
                   @"Nehemiah": @"",
                   @"Capernaum": @"",
                   @"Shimeah": @"",
                   @"Manaen": @"",
                   @"Shemeber": @"",
                   @"Machbenah": @"",
                   @"Tahpenes": @"",
                   @"Hadoram": @"",
                   @"Ahoah": @"",
                   @"Heber": @"",
                   @"Ephah": @"",
                   @"Dumah": @"",
                   @"Ashtaroth": @"",
                   @"Eben": @"-Ezer",
                   @"Joshua": @"",
                   @"Parah": @"",
                   @"Joel": @"",
                   @"Kareah": @"",
                   @"Parmashta": @"",
                   @"Epaphroditus": @"",
                   @"Migdal": @"-gad",
                   @"Hena": @"he has driven away",
                   @"Siddim": @"",
                   @"Ethnan": @"",
                   @"Sered": @"",
                   @"Pi": @"-hahiroth",
                   @"Teman": @"",
                   @"Maai": @"",
                   @"Shage": @"",
                   @"Hareph": @"",
                   @"Boson": @"",
                   @"Ananias": @"",
                   @"Lasea": @"",
                   @"Ophir": @"",
                   @"Luke": @"light-giving",
                   @"Shobek": @"",
                   @"Moab": @"",
                   @"Judah": @"praised celebrated praise",
                   @"Carpus": @"",
                   @"Abednego": @"servant of Nego perhaps the same as Nebo",
                   @"Ittah": @"-kazin",
                   @"Zoar": @"",
                   @"Bealoth": @"",
                   @"On": @"",
                   @"Aznoth": @"-tabor ears of Tabor",
                   @"Ezri": @"",
                   @"Jeaterai": @"whom YHWH leads",
                   @"Addon": @"lord",
                   @"Kohath": @"Obedience Assembly",
                   @"Naomi": @"",
                   @"Ira": @"watcher watchful city watch",
                   @"Matred": @"",
                   @"Meonenim": @"",
                   @"Shebna": @"",
                   @"Masrekah": @"",
                   @"Mebunnai": @"",
                   @"Calno": @"",
                   @"Hiel": @"",
                   @"Ephphatha": @"",
                   @"Thyatira": @"",
                   @"Jehosheba": @"YHWHs oath YHWH-swearing",
                   @"Naashon": @"",
                   @"Jerebai": @"",
                   @"Sapphira": @"",
                   @"Shalim": @"",
                   @"Ashbel": @"",
                   @"Gabriel": @"",
                   @"Bethabara": @"",
                   @"Zachariah": @"remembered by YHWH remembered by the Lord",
                   @"Michri": @"",
                   @"Jasher": @"",
                   @"Elhanan": @"",
                   @"Zoheth": @"",
                   @"Harumaph": @"",
                   @"Shebarim": @"",
                   @"Tarsus": @"",
                   @"Lysanias": @"",
                   @"Lo": @"-ruhamah",
                   @"Beor": @"burning or torch a torch",
                   @"Media": @"",
                   @"Asriel": @"",
                   @"Ekron": @"",
                   @"Abda": @"a servant",
                   @"Bartholomew": @"",
                   @"Unni": @"",
                   @"Libya": @"",
                   @"Moladah": @"",
                   @"Zarhites": @"",
                   @"Adalia": @"",
                   @"Jerahmeel": @"",
                   @"Pethahiah": @"(literally means Yah Opens, Yahweh Opens (The Womb) Loosened Of The Lord, Whom The Lord Set Free.)",
                   @"Tarshish": @"",
                   @"Antioch": @"",
                   @"Mahath": @"",
                   @"Rithmah": @"",
                   @"Halhul": @"",
                   @"En": @"-tappuah",
                   @"Haruz": @"",
                   @"Jehiah": @"YHWH lives",
                   @"Baalah": @"mistress",
                   @"Paseah": @"",
                   @"Huram": @"",
                   @"Sheshach": @"",
                   @"Eli": @"",
                   @"Laish": @"",
                   @"Macedonia": @"extended land",
                   @"Jaddua": @"very knowing",
                   @"Harran": @"",
                   @"Jetheth": @"",
                   @"Nophah": @"",
                   @"Teraphim": @"",
                   @"Zeboiim": @"",
                   @"Barachias": @"",
                   @"Uriel": @"",
                   @"Sherebiah": @"",
                   @"Mahanem": @"",
                   @"Perez": @"-Uzza",
                   @"Mishael": @"",
                   @"Taberah": @"",
                   @"Rhegium": @"",
                   @"Luz": @"",
                   @"Publius": @"",
                   @"Naioth": @"",
                   @"Thamah": @"",
                   @"Asaiah": @"the Lord has made",
                   @"Alammelech": @"",
                   @"Gedaliah": @"",
                   @"Haran": @"",
                   @"Bakbuk": @"a flagon hollow",
                   @"Ahithophel": @"brother of foolishness",
                   @"Naphtali": @"My wrestling",
                   @"Hazeroth": @"",
                   @"Salim": @"",
                   @"Shalmai": @"",
                   @"Zalaph": @"wound",
                   @"Esek": @"",
                   @"Emmaus": @"",
                   @"Troas": @"",
                   @"Zebulon": @"Dwelling",
                   @"Dothan": @"",
                   @"Nethaneel": @"",
                   @"Zophim": @"",
                   @"Shimei": @"",
                   @"Chisleu": @"Cisleu Casleu",
                   @"Amaziah": @"the strength of the Lord",
                   @"Ahinoam": @"",
                   @"Isaac": @"he laughed laughing one",
                   @"Hannah": @"",
                   @"Niger": @"",
                   @"Bernice": @"bearer of victory",
                   @"Jarib": @"",
                   @"Caiphas": @"",
                   @"Hashubah": @"",
                   @"Caleb": @"-Ephratah",
                   @"Puteoli": @"",
                   @"Ish": @"-pan firm strong",
                   @"Sela": @"-hammah-lekoth",
                   @"Berothai": @"",
                   @"Reuben": @"Behold a Son",
                   @"Abdi": @"my servant",
                   @"Sarepta": @"",
                   @"Sharar": @"",
                   @"Pharisees": @"",
                   @"Chelluh": @"",
                   @"Chios": @"",
                   @"Crete": @"",
                   @"Harnepher": @"",
                   @"Sheba": @"",
                   @"Eglon": @"",
                   @"Diblaim": @"",
                   @"Bilgah": @"cheerfulness",
                   @"Philologus": @" Lover Of The Word",
                   @"Pochereth": @"",
                   @"Nimrod": @"",
                   @"Bishlam": @"",
                   @"Sichem": @"",
                   @"Nicolas": @"",
                   @"Javan": @"",
                   @"Aholah": @"",
                   @"Bigvai": @"",
                   @"Jahath": @"revival or grasping",
                   @"Tibni": @"",
                   @"Kadesh": @"",
                   @"Patmos": @"",
                   @"Sacar": @"",
                   @"Aven": @"",
                   @"Azgad": @"Gad is strong",
                   @"Dan": @"",
                   @"Mehetabel": @"",
                   @"Enon": @"",
                   @"Elul": @"",
                   @"Upharsin": @"",
                   @"Tanhumeth": @"",
                   @"Abarim": @"regions beyond",
                   @"Remaliah": @"",
                   @"Jezer": @"power",
                   @"Gibeah": @"",
                   @"Abihail": @"father of i.e. possessing strength",
                   @"Beon": @"meaning uncertain",
                   @"Habaiah": @"",
                   @"Adar": @"high",
                   @"Nahbi": @"",
                   @"Hobah": @"",
                   @"Gezer": @"",
                   @"Yehezkel": @"",
                   @"Imlah": @"full God does fill fulfilling plenitude",
                   @"Ethanim": @"",
                   @"Adonijah": @"my lord is YHWH",
                   @"Gabbatha": @"",
                   @"Shophan": @"",
                   @"Keturah": @"",
                   @"Azekah": @"",
                   @"Nicopolis": @"",
                   @"Sheminith": @"",
                   @"Elam": @"",
                   @"Ammizabad": @"",
                   @"Kittim": @"",
                   @"Assur": @"same as Ashur",
                   @"Mishraites": @"",
                   @"Yehoyada": @"",
                   @"Hanes": @"",
                   @"Tammuz": @"",
                   @"Jehush": @"",
                   @"Ava": @"",
                   @"Hermas": @"",
                   @"Michal": @"",
                   @"Kabzeel": @"",
                   @"Jeriah": @"",
                   @"Hashupha": @"",
                   @"Eutychus": @"",
                   @"Hazaiah": @"",
                   @"Tryphena": @"",
                   @"Orpah": @"",
                   @"Cyprus": @"",
                   @"Mahalah": @"",
                   @"Jekuthiel": @"",
                   @"Alemeth": @"",
                   @"Neriah": @"",
                   @"Melzar": @"steward",
                   @"Mysia": @"",
                   @"Harhas": @"",
                   @"Ozias": @"",
                   @"Rakem": @"flower garden",
                   @"Lydda": @"",
                   @"Jehoiada": @"",
                   @"Zabdi": @"Gift of Iesus my gift",
                   @"Zophai": @"",
                   @"Betharabah": @"",
                   @"Asyncritus": @"incomparable",
                   @"Shenir": @"",
                   @"Zephathah": @"",
                   @"Jaziz": @"",
                   @"Putiel": @"",
                   @"Jacob": @"supplanter one who follows on anothers heels supplanter he that supplants or follows after supplanted",
                   @"Nibshan": @"",
                   @"Eliahba": @"",
                   @"Lysimachus": @"",
                   @"Zephath": @"",
                   @"Sychar": @"",
                   @"Jubal": @"",
                   @"Naaran": @"juvenile boyish juvenile",
                   @"Shemaiah": @"",
                   @"Aholiab": @"",
                   @"Pelaiah": @"",
                   @"Boanerges": @"",
                   @"Adoniram": @"",
                   @"Ecclesiasticus": @"or the Wisdom of Sirach",
                   @"Darius": @"he that informs himself of a king",
                   @"Rumah": @"",
                   @"Zoreah": @"",
                   @"Giddel": @"",
                   @"Tartak": @"",
                   @"Jesaiah": @"salvation of Iesus",
                   @"Hegai": @"or Hege",
                   @"Zartanah": @"",
                   @"Naham": @"",
                   @"Hadattah": @"new NEW HAZOR",
                   @"Pahath": @"-Moab",
                   @"Amok": @"",
                   @"Sheshbazzar": @"",
                   @"Hannathon": @"",
                   @"Ehi": @"",
                   @"Gazer": @"",
                   @"Maranatha": @"",
                   @"Ishui": @"Jesui equality",
                   @"Rachab": @"",
                   @"Ramath": @"-mizpeh",
                   @"Igal": @"may God redeem deliverer he will vindicate",
                   @"Zamzummims": @"",
                   @"Milcom": @"",
                   @"Gallim": @"",
                   @"Reu": @"",
                   @"Makkedah": @"",
                   @"Harosheth": @"",
                   @"Misham": @"",
                   @"Zeri": @"",
                   @"Noadiah": @"",
                   @"Arpad": @"",
                   @"Ismaiah": @"Iesus hears",
                   @"Lahairoi": @"",
                   @"Adadah": @"festival or boundary possible miswritten form of Aroer",
                   @"Quirinius": @"",
                   @"Arnon": @"",
                   @"Minnith": @"",
                   @"Darda": @"a pearl of wisdom",
                   @"Bakbukiah": @"wasted by Iesus effusion of Iesus",
                   @"Jaakan": @"he twists he shall surround intelligent",
                   @"Bethphage": @"",
                   @"Paul": @"small little",
                   @"Ed": @"",
                   @"Ithran": @"",
                   @"Ohad": @"Powerful  Joining Together I Shall Praise",
                   @"Japhlet": @"",
                   @"Nimshi": @"",
                   @"Rephaiah": @"",
                   @"Eneas": @"",
                   @"Migron": @"",
                   @"Letushim": @"",
                   @"Huri": @"",
                   @"Aeneas": @"(or Æneas)",
                   @"Rebekah": @"",
                   @"Chemarims": @"",
                   @"Homam": @"",
                   @"Aristarchus": @"the best ruler",
                   @"Jebusi": @"",
                   @"Bileam": @"",
                   @"Jehonathan": @"whom Iesus gave",
                   @"Chidon": @"",
                   @"Cornelius": @"",
                   @"Matthew": @"gift of Iesus  Also called LEVI gift of God",
                   @"Hinnom": @"",
                   @"Shammuah": @"",
                   @"Peniel": @"",
                   @"Elimelech": @"",
                   @"Tirhakah": @"",
                   @"Uriah": @"",
                   @"Chorazin": @"",
                   @"Hashabnah": @"",
                   @"Hushai": @"",
                   @"Shisha": @"",
                   @"Horeb": @"",
                   @"Jehoiakim": @"",
                   @"Ahiah": @"",
                   @"Eleazar": @"God Has Helped",
                   @"Tabitha": @"",
                   @"Mishma": @"",
                   @"Shephi": @"",
                   @"Reins": @"kidneys",
                   @"Ashur": @"black",
                   @"Tamah": @"",
                   @"Toah": @"",
                   @"Golgotha": @"",
                   @"Amminadab": @"",
                   @"Arimathea": @"",
                   @"Janoah": @"rest",
                   @"Abishag": @"",
                   @"Naphish": @"",
                   @"Heman": @"",
                   @"Jabal": @"Leading or Flowing",
                   @"Meshech": @"",
                   @"Anamim": @"",
                   @"Naaman": @"pleasantness",
                   @"Ball": @"-hanan the Lord is gracious",
                   @"Mamre": @"",
                   @"Eshkalon": @"",
                   @"Beno": @"his son",
                   @"Yakob": @"Yacob - Jacob",
                   @"Gadarenes": @"",
                   @"Onesiphorus": @"",
                   @"Joshaviah": @"",
                   @"Joiarib": @"",
                   @"Iri": @"Iesus is watcher",
                   @"Kishon": @"",
                   @"Elika": @"",
                   @"Ramah": @"",
                   @"Tryphon": @"",
                   @"Colosse": @"",
                   @"Peor": @"",
                   @"Sheshai": @"",
                   @"Hierapolis": @"",
                   @"Amal": @"labor",
                   @"Shalmaneser": @"",
                   @"Gilboa": @"",
                   @"Achab": @"",
                   @"Ginath": @"Ginnetho protection",
                   @"Zur": @"",
                   @"Azmon": @"",
                   @"Pithon": @"",
                   @"Jeshohaia": @"",
                   @"Azariah": @"",
                   @"Hanameel": @"",
                   @"Beeliada": @"",
                   @"Galal": @"",
                   @"Telassar": @"",
                   @"Izhar": @"Anointed He Is On Top He produces oil bright one olive oil",
                   @"Joezer": @"",
                   @"Matthanias": @"same as Mattaniah",
                   @"Genubath": @"",
                   @"Ziph": @"",
                   @"Hatita": @"",
                   @"Apphia": @"",
                   @"Abishai": @"father of (i.e. desirous of) a gift",
                   @"Helam": @"",
                   @"Gamul": @" Weaned, Recompensed",
                   @"Ulla": @"",
                   @"Bedad": @"",
                   @"Leshem": @"",
                   @"Ocran": @"",
                   @"Edom": @"",
                   @"Shua": @"",
                   @"Patrobas": @"",
                   @"Likhi": @"learned",
                   @"Arumah": @"height",
                   @"Iru": @"watch",
                   @"Adullam": @"",
                   @"Ezem": @"",
                   @"Zaphnath": @"-paaneah revealer of a secret the man to whom secrets are revealed",
                   @"Phenice": @"",
                   @"Ephron": @"",
                   @"Timon": @"",
                   @"Uzzi": @"",
                   @"Bera": @"",
                   @"Zenas": @"",
                   @"Hakkoz": @"the nimble",
                   @"Akrabbim": @"",
                   @"Carshena": @"meaning uncertain",
                   @"Amon": @"",
                   @"Kelaiah": @"",
                   @"Hades": @"hell, asked for",
                   @"Abishalom": @"",
                   @"Asaph": @"collector of the people",
                   @"Mallothi": @"",
                   @"Bezek": @"",
                   @"Punon": @"",
                   @"Topheth": @"place of burning",
                   @"Obil": @"",
                   @"Thamar": @"",
                   @"Ashkenaz": @"spreading fire",
                   @"Jogbehah": @"",
                   @"Tidal": @"",
                   @"Sheleph": @"",
                   @"Mibhar": @"",
                   @"Enoch": @"",
                   @"Abner": @"father of light",
                   @"Tema": @"",
                   @"Melchiah": @"",
                   @"Eshean": @"",
                   @"Selah": @"",
                   @"Zimri": @"",
                   @"Hittite": @"",
                   @"Kenaz": @"",
                   @"Machnadebai": @"",
                   @"Nemuel": @"",
                   @"Shishak": @"",
                   @"Jethlah": @"",
                   @"Maarath": @"",
                   @"Imri": @"speaking exalting bitter a lamb projecting eloquent",
                   @"Bether": @"",
                   @"Misheal": @"",
                   @"Seba": @"",
                   @"Tirza": @"Pleasantness",
                   @"Sephar": @"",
                   @"Sarai": @"",
                   @"Ahisamach": @"",
                   @"Naamah": @"",
                   @"Asa": @"physician cure",
                   @"Seraiah": @"",
                   @"Zippor": @"",
                   @"Baana": @"affliction",
                   @"Kenan": @"",
                   @"Nahash": @"",
                   @"Shallum": @"",
                   @"Jeroboam": @"",
                   @"Jarmuth": @"",
                   @"Zeeb": @"",
                   @"Ithamar": @"island of the palm-tree palm-coast palm tree",
                   @"Abidan": @"father of the judge",
                   @"Charran": @"",
                   @"Guni": @"",
                   @"Amraphel": @"",
                   @"Arcturus": @"",
                   @"Gilgal": @"",
                   @"Euphrates": @"",
                   @"Azaz": @"strong",
                   @"Bethsaida": @"",
                   @"Yuval": @"",
                   @"Tabbath": @"",
                   @"Harhaiah": @"",
                   @"Abishua": @"father of welfare i.e. fortunate",
                   @"Madon": @"",
                   @"Jachan": @"afflicting or troublous affliction",
                   @"Genesis": @"",
                   @"Dabbasheth": @"",
                   @"Azmaveth": @"",
                   @"Saul": @"asked of God",
                   @"Ishmachiah": @"Iesus supports may God support*Ishvah resembles",
                   @"Mahlah": @"",
                   @"Ziklag": @"",
                   @"Crispus": @"",
                   @"Perida": @"",
                   @"Nain": @"",
                   @"Punites": @"",
                   @"Raamah": @"",
                   @"Baali": @"My lord An appellation of Iesus",
                   @"Hammelech": @"",
                   @"Shulamite": @"",
                   @"Ethbaal": @"",
                   @"Abraham": @"father of a multitude",
                   @"Tobiah": @"",
                   @"Helem": @"",
                   @"Zaccai": @"pure",
                   @"Melatiah": @"",
                   @"Shiza": @"",
                   @"Japhet": @"",
                   @"Esther": @"",
                   @"Heldai": @"Heleb Heled",
                   @"Gederothaim": @"",
                   @"Etam": @"",
                   @"Aholibah": @"",
                   @"Sopater": @"",
                   @"Ebed": @"-melech",
                   @"Yosef": @"- Joseph",
                   @"Tire": @"headdress",
                   @"Shobach": @"",
                   @"Cesar": @"",
                   @"Maasiai": @"",
                   @"Gideon": @"",
                   @"Festus": @"festive joyful festal prosperous",
                   @"Shibmah": @"",
                   @"Baalis": @"lord of joy rules",
                   @"Jose": @"",
                   @"Aiath": @"",
                   @"Hepher": @"",
                   @"Maadiah": @"a priest",
                   @"Chittem": @"",
                   @"Maroth": @"",
                   @"Ahilud": @"",
                   @"Zithri": @"",
                   @"Chalcol": @"meaning uncertain",
                   @"Rabbi": @"",
                   @"Agia": @"(Greek variant of Hebrew Hattil)",
                   @"Ur": @"",
                   @"Methuselah": @"",
                   @"Higgaion": @"",
                   @"Colhozeh": @"",
                   @"Maralah": @"",
                   @"Rhoda": @"",
                   @"Timnath": @"-heres",
                   @"Kemuel": @"",
                   @"Jidlaph": @"",
                   @"Clement": @"",
                   @"Andronicus": @"",
                   @"Ribai": @"",
                   @"Amana": @"",
                   @"Fortunatus": @"lucky fortunate prosperous",
                   @"Lapidoth": @"",
                   @"Rizpah": @"",
                   @"Laadan": @"",
                   @"Karkaa": @"",
                   @"Heresh": @"",
                   @"Medan": @"",
                   @"Shadrach": @"",
                   @"Maharai": @"",
                   @"Seneh": @"",
                   @"Omri": @"",
                   @"Adoram": @"",
                   @"Anathema": @"",
                   @"Zabbai": @"wanderer pure pure",
                   @"Gebal": @"",
                   @"Bigthan": @"",
                   @"Messiah": @"",
                   @"Zarephath": @"",
                   @"Dibri": @"",
                   @"Senaah": @"",
                   @"Zephonites": @"",
                   @"Shalisha": @"",
                   @"Jerah": @"",
                   @"Hazael": @"",
                   @"Issachar": @"rewarded Purchased me",
                   @"Jemima": @"",
                   @"Rabboni": @"lord teacher",
                   @"Esli": @"",
                   @"Anaiah": @"whom Iesus answers",
                   @"Abiud": @"father of praise",
                   @"Lasha": @"fissure",
                   @"Hanan": @"",
                   @"Ethan": @"",
                   @"Remmon": @"",
                   @"Jether": @"",
                   @"Zebadiah": @"",
                   @"Toi": @"who wanders",
                   @"Anaharath": @"",
                   @"Aristobulus": @"the best counsellor",
                   @"Maachathi": @"",
                   @"Obadiah": @"",
                   @"Lod": @"",
                   @"Kishi": @"",
                   @"Dan": @"Judge",
                   @"Merari": @"My Bitterness or My Strength",
                   @"Hezron": @"Enclosure or Surrounded By A Wall",
                   @"Uzziel": @" God Is My Strength",
                   @"Canaanite": @"Merchant or Servant",
                   @"Shimi": @"the name of a number of Isr To hear",
                   @"Mahli": @"Weak Silly",
                   @"Shemaiah": @"Yahweh Has Heard Heard Of The Lord",
                   @"Huppah": @"covering",
                   @"Hezir": @"( A bog, converted, swine or protected)(?)",
                   @"Happizzez": @"The Scattering. Dispersion",
                   @"Maziah": @"consolation of Iesus"
                   };
    
    // Do any additional setup after loading the view.
}
- (IBAction)TranslateButton:(id)sender {


    self.string = [NSMutableString stringWithString: self.InputText.stringValue];
    for (NSString *key in self.dict) {
        
        NSString *def = [self.dict valueForKey:key];
        NSString *pattern = [NSString stringWithFormat: @"\\b%@\\b", key];
        NSString *defpattern = [NSString stringWithFormat:@"\n $0: (%@)\n", def];
        
        NSError *error = NULL;
        
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:pattern options:NSRegularExpressionCaseInsensitive error:&error];
        
        NSString *newString =
        [regex stringByReplacingMatchesInString:self.string options:0 range:NSMakeRange(0, [self.string length])  withTemplate:defpattern]
        ;
        
        self.string = [NSMutableString stringWithString: newString];
        
        self.OutputText.string = self.string;
        
        //NSLog(@"key:%@\ndef:%@", key, def);
        
    };
    //[self.InputText resignFirstResponder];

    
}


- (void)setRepresentedObject:(id)representedObject {
    [super setRepresentedObject:representedObject];

    // Update the view, if already loaded.
}


@end
